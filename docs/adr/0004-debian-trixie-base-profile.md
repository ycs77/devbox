...
ADR-0037 supersedes the Debian 13 Trixie Slim profile choice and all Trixie-specific package, ABI, compatibility, size, layer, and prototype evidence with Ubuntu 24.04 LTS. ADR-0035 separately supersedes the local Base image, Base `image_id`, explicit Base materialization, and independent Base-update lifecycle. The single versioned Base-profile role plus the curated native contents, timezone, locale, non-root, and no-host-identity intentions remain in force and must be re-resolved and verified for Ubuntu inside the common Workspace Dockerfile.
